...
def init_db():
    ...
    # Table updated with 'notification_link'
...
<body>
    ...
    <div class="main-nav"><a href="/">HOME</a><a href="/category/Latest Job">LATEST JOBS</a><a href="/category/Admit Card">ADMIT CARD</a><a href="/category/Result">RESULT</a></div>

    <div class="container mt-3">
        {% if mode == 'home' %}
            <div class="row">
                {% for cat in ['Result', 'Admit Card', 'Latest Job'] %}
                <div class="col-md-4"><div class="box"><div class="box-head">{{ cat }}</div>
                {% for item in data if item[1] == cat %}<a href="/notice/{{ item[0] }}" class="job-link">{{ item[2] }}</a>{% endfor %}
                </div></div>{% endfor %}
            </div>
        ...
        {% elif mode == 'detail' %}
            <div class="card p-3 shadow-sm bg-white">
                ...
                <table class="job-table">
                    ...
                    <tr>
                        <td><span class="label">Apply Start:</span> {{ item[3] }}<br><span class="label">Last Date:</span> {{ item[4] }}</td>
                        <td><span class="label">Gen/OBC/EWS:</span> Rs. {{ item[6] }}/-<br><span class="label">SC/ST:</span> Rs. {{ item[7] }}/-</td>
                    </tr>
                    <tr class="sub-header"><td colspan="2">Vacancy Details & Eligibility</td></tr>
                    <tr class="text-center"><td class="label">Post Name & Total</td><td class="label">Eligibility Criteria</td></tr>
                    <tr>
                        <td class="text-center"><b>Total: {{ item[10] }}</b></td>
                        <td><ul>{% for line in item[11].split('\\n') if line.strip() %}<li>{{ line.strip() }}</li>{% endfor %}</ul></td>
                    </tr>
                    <tr class="header-row"><td colspan="2">Important Links</td></tr>
                    <tr class="text-center"><td><span class="label">Official Notification</span></td><td><a href="{{ item[13] }}" target="_blank" class="btn btn-warning btn-sm">Download PDF</a></td></tr>
                    <tr class="text-center"><td><span class="label">Action Link</span></td><td><a href="{{ item[12] }}" target="_blank" class="btn btn-primary btn-sm">Click Here</a></td></tr>
                </table>
            </div>

        {% elif mode == 'admin' %}
            <div class="card p-4 shadow">
                <h4>{% if edit_item %}Edit Notice{% else %}Add New Notice{% endif %}</h4>
                <form method="POST" action="{% if edit_item %}/edit/{{ edit_item[0] }}{% else %}/admin{% endif %}">
                    <div class="row g-2">
                        ...
                        <div class="col-md-8"><input name="title" value="{{ edit_item[2] if edit_item }}" placeholder="Notice Title" class="form-control" required></div>
                        <div class="col-md-4"><input name="start" value="{{ edit_item[3] if edit_item }}" placeholder="Start Date" class="form-control"></div>
                        <div class="col-md-4"><input name="last" value="{{ edit_item[4] if edit_item }}" placeholder="Last Date" class="form-control"></div>
                        <div class="col-md-4"><input name="f_gen" value="{{ edit_item[6] if edit_item }}" placeholder="Fee Gen" class="form-control"></div>
                        <div class="col-md-4"><input name="f_sc" value="{{ edit_item[7] if edit_item }}" placeholder="Fee SC/ST" class="form-control"></div>
                        <div class="col-md-4"><input name="total" value="{{ edit_item[10] if edit_item }}" placeholder="Total Posts" class="form-control"></div>
                        <div class="col-md-6"><input name="link" value="{{ edit_item[12] if edit_item }}" placeholder="Apply/Result URL" class="form-control"></div>
                        <div class="col-md-6"><input name="n_link" value="{{ edit_item[13] if edit_item }}" placeholder="Notification PDF Link" class="form-control"></div>
                        <div class="col-12"><textarea name="elig" placeholder="Eligibility (Enter for bullet points)" class="form-control" rows="4">{{ edit_item[11] if edit_item }}</textarea></div>
                    </div>
                    <button class="btn btn-success mt-2 w-100">{% if edit_item %}Update Notice{% else %}Publish Notice{% endif %}</button>
                </form>
                <hr>
                <h5>Manage Posts</h5>
                <table class="table table-sm">
                    {% for i in data %}
                    <tr><td>{{ i[2] }}</td><td><a href="/edit/{{ i[0] }}">Edit</a> | <a href="/delete/{{ i[0] }}" class="text-danger">Del</a></td></tr>
                    {% endfor %}
                </table>
            </div>
        {% elif mode == 'login' %}
            <form method="POST" class="card p-4 mx-auto mt-5" style="max-width:350px;">
                <h4>Admin Login</h4>
                <input name="user" placeholder="User" class="form-control mb-2"><input name="pass" type="password" placeholder="Pass" class="form-control mb-2">
...
def detail(id):
    ...
    conn.close()
    return render_template_string(HTML_LAYOUT, mode='detail', item=item)
